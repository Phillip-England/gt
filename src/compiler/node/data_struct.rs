...
impl DataStruct {
    pub fn new(toks: Vec<AdvancedToken>) -> Result<DataStruct, ErrParser> {
        ...
        if second_tok_opt.is_none() {
            return fail!(
                ErrParser::MalformedDataStruct,
                "attempted to access a token containing data struct name but could not locate it"
            );
        }
        ...
        if let AdvancedToken::Indicator(s) = second_tok {
            ...
            return fail!(
                ErrParser::ExpectedIndicatorToken,
                "attempted to access tokens for ast generation and expected and indicator token but could not find one"
            );
        }
        ...
        let field_toks: Vec<AdvancedToken> = toks
            .into_iter()
        ...
        loop {
            ...
            if name_opt.is_none() {
                return fail!(
                    ErrParser::MalformedDataStruct,
                    "could not find field name in expected location"
                );
            }
            ...
            if t_opt.is_none() {
                return fail!(
                    ErrParser::MalformedDataStruct,
                    "could not find field type in expected location"
                );
            }
            let t = t_opt.unwrap();
            ...
            match field.data_type {
                ...
                DataType::Custom(substruct_name) => {
                    let inner_struct = match data_struct_map.get(&substruct_name) {
                        ...
                        None => {
                            return fail!(
                                ErrParser::InvalidStructAccess,
                                "attempted to access a struct named {} but it does not exist",
                                substruct_name.clone()
                            );
                        }
                    };
                    ...
                    if vec.contains(&inner_struct_name) {
                        return fail!(
                            ErrParser::InfiniteStructReference,
                            "the struct named {} has an infinite reference loop",
                            parent.name
                        );
                    }
                        ...
                        DataType::Custom(s) => {
                            ...
                            let inner_struct = match data_struct_map.get(&array_data_type_name) {
                                ...
                                None => {
                                    return fail!(
                                        ErrParser::InvalidStructAccess,
                                        "the struct named {} does not exist",
                                        parent.name
                                    );
                                }
                            };
                            ...
                            vec =
                                inner_struct.get_substruct_names(vec, data_struct_map, &parent)?;
                            let inner_struct_name = inner_struct.name.clone();
                            if vec.contains(&inner_struct_name) {
                                return fail!(
                                    ErrParser::InfiniteStructReference,
