...
impl Variable {
    ...
    pub fn new(toks: Vec<AdvancedToken>) -> Result<Variable, AppErr> {
        ...
        match tok1 {
            ...
            _ => {
                return Err(app_err!(AppErrKind::Parser(ParserErr::MalformedVariable(String::from("expected 1st token from 'let' keyword to be of type Indicator")))));
            }
        }
        let tok2 = l.peek(2);
        if !matches!(tok2, AdvancedToken::Colon) {
            return Err(app_err!(AppErrKind::Parser(ParserErr::MalformedVariable(String::from("expected 2nd token from 'let' keyword to be of type Colon")))));
        }
        let tok3 = l.peek(3);
        match tok3 {
            ...
            _ => {
                return Err(app_err!(AppErrKind::Parser(ParserErr::MalformedVariable(String::from("expected 3rd token from 'let' keyword to be of one of the following types: Indicator, Str, Bool, or Num")))));
            }
        }
        let tok4 = l.peek(4);
        if !matches!(tok4, AdvancedToken::OperatorAssignment) {
            return Err(app_err!(AppErrKind::Parser(ParserErr::MalformedVariable(String::from("expected 4th token from 'let' keyword to be of type OperatorAssignment")))))
        }
        ...
        loop {
            ...
            if l.at_end() {
                return Err(app_err!(AppErrKind::Parser(ParserErr::MalformedVariable(String::from("failed to located a semicolon for variable")))));
            }
            l.next();
        }
        // value extraction
        let value_toks = l.collect(l.marked_pos(), l.pos());
        let value: String = value_toks.into_iter().filter_map(|t| {
            match t {
